---
name: skill-forge
description: "Create high-quality, production-grade skills for Claude Code. Expert guidance on skill architecture, workflow design, prompt engineering, and packaging. Use when user wants to create a new skill, build a skill, design a skill, write a skill, update an existing skill, improve a skill, refactor a skill, debug a skill, or package a skill. Specifically: 'create skill', 'build skill', 'new skill', 'skill creation', 'write a skill', 'make a skill', 'design a skill', 'improve skill', 'package skill', 'skill development', 'skill template', 'skill best practices', 'write SKILL.md', 'upgrade my skills', 'skill migration'. Step-by-step workflow: understand purpose -> plan architecture -> initialize (python scripts/init_skill.py) -> write description -> write SKILL.md body -> build resources -> review -> package (python scripts/package_skill.py). Iron Law: every line must justify its token cost."
---
...
- [ ] Step 3: Initialize ⛔ BLOCKING (skip if skill already exists)
  - [ ] Run `python scripts/init_skill.py <skill-name> --path <output-directory>` (Windows: `python`, not `python3`)
...
- [ ] Step 8: Package
  - [ ] Run `python scripts/package_skill.py <path/to/skill-folder> [output-directory]` (Windows: `python`, not `python3`)
