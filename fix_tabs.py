...
for i, line in enumerate(lines):
    if '<Tabs defaultActiveKey="lifecycle_mining"' in line:
        start_idx = i
...
if start_idx != -1 and end_idx != -1:
    ...
    # Inject state after vibeLoading
    for i, line in enumerate(lines):
        if 'const [vibeLoading, setVibeLoading] = useState(false);' in line:
            lines.insert(i+1, state_injection)
            break
            
    # Now indices might have shifted by 1
    # Let's re-find them
    start_idx = -1
    end_idx = -1
    for i, line in enumerate(lines):
    ...
    lines = lines[:start_idx] + [new_tabs] + lines[end_idx+1:]
